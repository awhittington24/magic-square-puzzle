        ...
        try:
            ...
            if 0 < row <= 7 and 0 < col <= 7:
                ...
                if array7[row-1][col-1] == '-' and is_adjacent(row, col, predecessor):
                    backup_array = [row[:] for row in array]  # Create a deep copy of the array7 for undo
                    undo_stack.append((row, col, backup_array))

                    array7[row - 1][col - 1] = str(current_num_label.cget('text'))
                    display_array7_gui(array, array7, label)

                    # Check if the number is on a diagonal
                    if 1 <= row <= 5 and 1 <= col <= 5:
                        diagonal_index = row + col - 2

                        # Check if the number can be placed on the diagonal
                        if (array7[diagonal_index][0] == '-' and array7[diagonal_index][0] != '1') or (
                                array7[diagonal_index][6] == '-' and array7[diagonal_index][6] != '1'):
                            array7[diagonal_index][0 if col == 1 else 6] = str(current_num_label.cget('text'))
                            display_array7_gui(array, array7, label)
...
submit_button = tk.Button(root, text="Submit",
                          command=lambda: handle_input([row_entry, col_entry], label, current_num_label, root,
                                                       undo_button) if level == 1 else handle2([row_entry, col_entry], label, current_num_label, root,
                                                                                             undo_button))
submit_button.pack()
